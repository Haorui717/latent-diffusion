...
config_paths=(
    "$config_root/haorui/ldm_unconditional/ldm_unconditional_n_1800.yaml"
)
output_root="/home/yixiao/haorui/ccvl15/haorui/latent-diffusion-Local/tmux_log"
output_paths=(
    "$output_root/ldm_unconditional/ldm_unconditional_n_1800_$(date +"%Y:%m:%d:%H:%M:%S").txt"
)

# An array of commands you want to run
declare -a commands=(
    "conda activate ldm; python main.py --base ${config_paths[0]} -t --logdir logs/ldm_unconditional --gpu 0,1,2,3,4,5,6,7 2>&1 | tee ${output_paths[0]}"
)

declare -a session_names=(
    "ldm_unconditional_n_1800"
    # "ldm_polyp_1000"
    # "ldm_polyp_500"
)
